...
<div class="card" style="border-radius:20px;padding:20px;">
    ...
    <div style="margin-top:16px;">
        ...
        <p><strong>Phone:</strong> {{ $parent->phone_number ?? Auth::user()->phone_number ?? 'N/A' }}</p>
        ...
        <p><strong>Phone:</strong> {{ $secondParent->phone_number ?? 'N/A' }}</p></p>
        @endif
        @if($guardian)
        <hr>
        <h5>Guardian</h5>
        <p><strong>Name:</strong> {{ $guardian->name }}</p>
        <p><strong>Phone:</strong> {{ $guardian->phone_number ?? 'N/A' }}</p></p>
        @endif
    </div>
</div>
@endsection
